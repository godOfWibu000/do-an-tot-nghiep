...
@section('content')
    <div class="content p-2">
        ...
        <div class="cho-o-container">
            <h3 class="border-bottom-1 width-60-percent">Chỗ ở hàng đầu</h3>
            <div>
                <button class="area-button button button-radius-20 back-color-main-1 color-white hover-opacity-08-05" onclick="getChoOHangDau('{{ route('cho-o.cho-o-hang-dau', ['area' => 'hà Nội' ]) }}', this)">Hà Nội</button>
                @foreach (getTopHotelsForArea() as $item)
                    <button class="area-button button button-radius-20 color-main-1 hover-opacity-08-05" onclick="getChoOHangDau('{{ route('cho-o.cho-o-hang-dau', ['area' => $item ]) }}', this)">{{ $item }}</button>
                @endforeach
            </div>
            <br>
            <div class="ds-cho-nghi-noi-bat flex flex-wrap" id="ds-cho-nghi-noi-bat">
                @foreach ($hotelsList as $key => $value)
                    <div class="cho-nghi-noi-bat back-color-white p-1">
                        <div class="box-shadow-10-black border-radius-10 position-relative cursor">
                            @if ($checkLogin)
                                @if($checkSaveHotels1[$key] && $checkSaveHotels1[$key] != null)
        ...
        <div class="cho-o-container">
            <h3 class="border-bottom-1 width-60-percent">Chỗ ở mới</h3>
            <br>
            <div class="ds-cho-nghi-noi-bat flex flex-wrap">
                @foreach ($newHotelsList as $key => $value)
                    <div class="cho-nghi-noi-bat back-color-white p-1">
                        <div class="box-shadow-10-black border-radius-10 position-relative cursor">
                            @if ($checkLogin)
                                @if($checkSaveHotels2[$key] && $checkSaveHotels2[$key] != null)
                                    <i title="Bỏ lưu" class="fas fa-bookmark fs-4 text-warning position-absolute save-hotel" id="unsave-{{ $value->hotel_id }}" onclick="luuChoO('{{ $value->hotel_id }}', '{{ route('danh-gia-va-da-luu.luu-cho-o') }}', '{{ route('danh-gia-va-da-luu.bo-luu-cho-o', ['id' => $value->hotel_id]) }}', '{{ csrf_token() }}', this)"></i>
                                @else
                                    <i title="Lưu lại" class="far fa-bookmark fs-4 text-warning position-absolute save-hotel" id="save-{{ $value->hotel_id }}" onclick="luuChoO('{{ $value->hotel_id }}', '{{ route('danh-gia-va-da-luu.luu-cho-o') }}', '{{ route('danh-gia-va-da-luu.bo-luu-cho-o', ['id' => $value->hotel_id]) }}', '{{ csrf_token() }}', this)"></i>
                                @endif
